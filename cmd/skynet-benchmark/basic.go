...
const (
	// The TurtleDexPath that will be used by the program to upload files for the
	// basic check.
	testTurtleDexDirBasic = "var/skynet-benchmark-basic"
)
...
func basicULDL(fileSize uint64) error {
	...
	baseTurtleDexPath, err := modules.NewTurtleDexPath(testTurtleDexDirBasic)
	if err != nil {
		return errors.AddContext(err, "error creating base sia path")
	}
	sp, err := baseTurtleDexPath.Join(name + "kb")
	if err != nil {
